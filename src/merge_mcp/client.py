...
class MergeAPIClient:
    ...
    def __new__(
        cls, api_key: Optional[str] = None, account_token: Optional[str] = None
    ):
        """
    ...
    def __init__(
        self, api_key: Optional[str] = None, account_token: Optional[str] = None
    ):
        """
        ...
        self._tenant = (
            os.getenv("MERGE_TENANT").upper() if os.getenv("MERGE_TENANT") else "US"
        )
        ...
        if not self._api_key:
            raise MergeClientError(
                "Merge API key is required. Set MERGE_API_KEY environment variable or pass it to the constructor."
            )

        if not self._account_token:
            raise MergeClientError(
                "Merge account token is required. Set MERGE_ACCOUNT_TOKEN environment variable or pass it to the constructor."
            )
    ...
    @classmethod
    async def get_initialized_instance(
        cls, api_key: Optional[str] = None, account_token: Optional[str] = None
    ) -> "MergeAPIClient":
        """
    ...
    async def fetch_enabled_scopes(self) -> List[CommonModelScope]:
        ...
        try:
            response = await self._make_request(
                "GET", "linked-account-scopes", with_retry=True, max_retries=2
            )
            ...
            for common_model in response["common_models"]:
                ...
                scopes.append(
                    CommonModelScope(
                        ...
                        is_read_enabled=model_permissions.get(
                            READ_SCOPE.upper(), {}
                        ).get("is_enabled", False),
                        is_write_enabled=model_permissions.get(
                            WRITE_SCOPE.upper(), {}
                        ).get("is_enabled", False),
                    )
                )
    ...
    async def get_openapi_schema(self) -> Dict[str, Any]:
        ...
        try:
            response = await self._make_request(
                "GET", "schema", unauthorized=True, with_retry=True
            )
            return response
        except MergeClientError as e:
            raise MergeClientError(f"Failed to get OpenAPI schema: {str(e)}") from e

    async def call_associated_meta_endpoint(
    ...
    async def _make_request(
        ...
        logger.debug(
            f"Initiating {method} request to {url} with params={params} and data={data}"
        )
            ...
            for attempt in range(1, attempts + 1):
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Response content is not in JSON format: {response.text[:100]}..."
                        )
                    ...
                    error_message = self._format_error_message(
                        ...
                        f"Response: {response_text or str(e)}",
                        attempt,
                        attempts,
                        method,
                        url,
