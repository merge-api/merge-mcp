...
class ScopeManager:
    ...
    def __init__(
        self, enabled_scopes: List[CommonModelScope], requested_scopes: List[str]
    ):
        """
    ...
    def get_available_scopes(self) -> List[CommonModelScope]:
        ...
        for scope in self.enabled_scopes:
            ...
            filtered_scope = CommonModelScope(
                ...
                is_read_enabled=scope.is_read_enabled
                and (READ_SCOPE in requested_operations),
                is_write_enabled=scope.is_write_enabled
                and (WRITE_SCOPE in requested_operations),
            )
            ...
            if filtered_scope.is_read_enabled or filtered_scope.is_write_enabled:
                ...
                logger.debug(
                    f"Added scope: {filtered_scope.model_name} (read: {filtered_scope.is_read_enabled}, write: {filtered_scope.is_write_enabled})"
                )
