...
class ToolManager:
    ...
    @classmethod
    async def create(
        cls, schema_index: SchemaIndex = None, requested_scopes: List[str] = None
    ) -> "ToolManager":
    ...
    async def _convert_operation_schema_to_tools(
        self, operation_schema: Dict[str, Any], is_creating_meta_tool: bool = False
    ) -> List[Tool]:
        """
        ...
        input_schema = self._schema_parser.extract_input_schema_and_update_parameters(
            operation_schema, self._schema_index
        )

        if self._needs_model_schema(operation_schema, input_schema):
            associated_meta_operation_schema = (
                self._get_associated_meta_tool_operation_schema(operation_schema)
            )
            if self._can_call_associated_meta_operation_schema(
                associated_meta_operation_schema
            ):
                try:
                    await self._populate_model_schema_with_meta_call(
                        operation_schema, input_schema
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to populate model schema with meta call for {name}: {e}"
                    )
                    return []
            if self._should_meta_tool_be_added(associated_meta_operation_schema):
                associated_meta_operation_schema[
                    "description"
                ] = f"{associated_meta_operation_schema['description']}\n\nThis operation should be called before {name}."
                associated_meta_tools = await self._convert_operation_schema_to_tools(
                    associated_meta_operation_schema, is_creating_meta_tool=True
                )
                associated_meta_tool = associated_meta_tools[0]
    ...
    def _needs_model_schema(
        self, operation_schema: Dict[str, Any], input_schema: Dict[str, Any]
    ) -> bool:
        """
        ...
        """
        return (
            operation_schema.get("method") in ["post", "patch"]
            and "model" in input_schema["properties"]
        )

    def _can_call_associated_meta_operation_schema(
        self, operation_schema: Dict[str, Any]
    ) -> bool:
        """
        Determine if an associated meta tool can be called. This is true when the meta tool has no required parameters outside of the headers.

        Args:
            operation_schema: The operation schema from the OpenAPI schema.

        Returns:
            A boolean indicating if an associated meta tool can be called.
        """
        return (
            len(
                [
    ...
    def _should_meta_tool_be_added(self, operation_schema: Dict[str, Any]) -> bool:
        """
        Determine if a meta tool should be added. This is true when the operation has parameters outside of the headers.

        Args:
            operation_schema: The operation schema from the OpenAPI schema.

        Returns:
            A boolean indicating if a meta tool should be added.
        """
        return (
            len(
                [
    ...
    async def _populate_model_schema_with_meta_call(
        self, operation_schema: Dict[str, Any], input_schema: Dict[str, Any]
    ) -> None:
        """
        ...
        meta_schema = await self._client.call_associated_meta_endpoint(
            operation_schema["endpoint"], operation_schema["method"]
        )
        input_schema_properties.update(
            meta_schema.get("request_schema", {}).get("properties", {})
        )
        return None

    def _format_associated_meta_tool_operation_id(
        self, operation_schema: Dict[str, Any]
    ) -> str:
        """
        Format the associated meta tool operation ID.

        Args:
            operation_schema: The operation schema from the OpenAPI schema.

        Returns:
            The formatted associated meta tool name.
        """
        name = operation_schema["operationId"]
        ...
        return f"{split_name[0]}_meta_{operation_schema['method']}_retrieve"

    def _get_associated_meta_tool_operation_schema(
        self, operation_schema: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Get the associated meta tool operation schema.

        Args:
            operation_schema: The operation schema from the OpenAPI schema.

        Returns:
            The associated meta tool operation schema.
        """
        associated_meta_operation_id = self._format_associated_meta_tool_operation_id(
            operation_schema
        )
        operation_schema = self._schema_index.get_by_operation_id(
            associated_meta_operation_id
        )
        if not operation_schema:
            raise ValueError(
                f"Associated meta operation not found: {associated_meta_operation_id}"
            )
        return operation_schema

    def _build_request_from_schema_and_arguments(
        ...
        request_meta = RequestMeta(method=method, path=path)
        ...
        for parameter in schema.get("parameters", []):
            ...
            if (
                ...
                or parameter.get("name") in EXCLUDED_PARAMETERS
            ):
                continue
            ...
            if param_location in ["query", "body", "path"]:
                ...
                if param_value is not None:
                    if param_location == "query":
                        ...
                        request_meta.path = request_meta.path.replace(
    ...
    async def call_tool(
        ...
        try:
            response = await self._client._make_request(
                ...
                headers=DEFAULT_TOOL_CALL_HEADERS,
                with_retry=True,
            )
            return [
                TextContent(
                    type="text",
                    text=f"Successfully called {name}, response: {response}",
                )
            ]
        except Exception as e:
            return [
                TextContent(
                    type="text",
                    text=f"Failed to call {name} with request: {request}, used schema: {schema}, and arguments: {arguments}, error: {str(e)}",
                )
            ]
