...
def test_build_component_properties_map(mock_openapi_schema, available_scopes):
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        assert (
            component_properties_map["Ticket"]["properties"]["id"]["type"] == "string"
        )
        assert (
            component_properties_map["Ticket"]["properties"]["title"]["type"]
            == "string"
        )
        assert (
            component_properties_map["Comment"]["properties"]["id"]["type"] == "string"
        )
        assert (
            component_properties_map["Comment"]["properties"]["body"]["type"]
            == "string"
        )
...
def test_create_tag_from_model():
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        assert (
            schema_index._create_tag_from_model("TimeOffRequest") == "time-off-requests"
        )
...
def test_build_index_for_available_scopes(mock_openapi_schema, available_scopes):
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        assert index["tickets"]["get"][0]["operationId"] in [
            "tickets_list",
            "multi_tag_list",
        ]
        assert index["tickets"]["post"][0]["operationId"] == "tickets_create"
        assert index["comments"]["get"][0]["operationId"] in [
            "comments_list",
            "multi_tag_list",
        ]
...
def test_build_operation_id_to_schema_map(mock_openapi_schema, available_scopes):
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        # Check that the schemas were added correctly
        assert (
            operation_id_to_schema_map["tickets_list"]["operationId"] == "tickets_list"
        )
        assert (
            operation_id_to_schema_map["tickets_create"]["operationId"]
            == "tickets_create"
        )
        assert (
            operation_id_to_schema_map["comments_list"]["operationId"]
            == "comments_list"
        )
        assert operation_id_to_schema_map["users_list"]["operationId"] == "users_list"
        assert (
            operation_id_to_schema_map["time_off_list"]["operationId"]
            == "time_off_list"
        )
        assert (
            operation_id_to_schema_map["multi_tag_list"]["operationId"]
            == "multi_tag_list"
        )
...
def test_update_schema_parameters_by_operation_id(
    mock_openapi_schema, available_scopes
):
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        schema_index.update_schema_parameters_by_operation_id(
            "tickets_list", new_parameters
        )
...
def test_get_schema_component_properties(mock_openapi_schema, available_scopes):
    ...
    with patch("merge_mcp.schema_index.MergeAPIClient") as mock_client_class:
        ...
        component_properties = schema_index.get_schema_component_properties(
            "NonExistent"
        )
