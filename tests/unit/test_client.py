...
def test_initialization_with_explicit_params(monkeypatch):
    ...
    client = MergeAPIClient(
        api_key="explicit_api_key", account_token="explicit_account_token"
    )
...
async def test_get_account_details(
    merge_client, mock_httpx_client, mock_account_details_response
):
...
@pytest.mark.asyncio
async def test_get_account_details_error(
    merge_client, mock_httpx_client, mock_http_error
):
...
@pytest.mark.asyncio
async def test_initialize_account_info(
    merge_client, mock_httpx_client, mock_account_details_response
):
...
@pytest.mark.asyncio
async def test_fetch_enabled_scopes(
    merge_client, mock_httpx_client, mock_enabled_scopes_response
):
...
@pytest.mark.asyncio
async def test_get_openapi_schema(
    merge_client, mock_httpx_client, mock_openapi_schema_response
):
    ...
    assert (
        request_args["headers"].get("Authorization") is None
...
def test_construct_meta_endpoint(merge_client):
    ...
    # Simple endpoint
    assert (
        merge_client._construct_meta_endpoint("/tickets", "get") == "/tickets/meta/get"
    )

    # Endpoint with ID
    assert (
        merge_client._construct_meta_endpoint("/tickets/123", "get")
        == "/tickets/meta/get/123"
    )

    # Complex endpoint
    assert (
        merge_client._construct_meta_endpoint("/tickets/123/comments", "get")
        == "/tickets/meta/get/123/comments"
    )
...
def test_format_url(merge_client):
    ...
    assert (
        merge_client._format_url("/api/hris/v1/employees")
        == "https://api.merge.dev/api/hris/v1/employees"
    )

    # Relative path with category
    assert (
        merge_client._format_url("employees")
        == "https://api.merge.dev/api/hris/v1/employees"
    )
    assert (
        merge_client._format_url("/employees")
...
async def test_session_id(merge_client):
    ...
    with patch.dict(
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            call_kwargs = (
                mock_client.return_value.__aenter__.return_value.request.call_args[1]
            )
