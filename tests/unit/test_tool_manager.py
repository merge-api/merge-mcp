...
async def test_create(mock_schema_index, mock_client):
    ...
    with patch(
        "merge_mcp.tool_manager.MergeAPIClient.get_instance", return_value=mock_client
    ):
        with patch("merge_mcp.tool_manager.ScopeManager") as mock_scope_manager_class:
            ...
            with patch.object(ToolManager, "async_init") as mock_async_init:
                tool_manager = await ToolManager.create(
                    mock_schema_index, ["ticket.read"]
                )
                ...
                assert (
                    tool_manager._enabled_scopes
                    == mock_client.fetch_enabled_scopes.return_value
                )
...
async def test_async_init(mock_schema_index):
    ...
    with patch(
        "merge_mcp.tool_manager.MergeAPIClient.get_instance", return_value=MagicMock()
    ):
...
async def test_fetch_tools(mock_schema_index):
    ...
    with patch(
        ...
        with patch.object(
            tool_manager, "_convert_operation_schema_to_tools"
        ) as mock_convert:
            ...
            assert mock_convert.call_count == len(
                mock_schema_index.get_all_operation_schemas()
            )
...
@pytest.mark.asyncio
async def test_convert_operation_schema_to_tools_basic(
    mock_schema_index, mock_schema_parser
):
    ...
    with patch(
        ...
        mock_schema_parser.extract_input_schema_and_update_parameters.return_value = (
            input_schema
        )
        ...
        with patch.object(tool_manager, "_needs_model_schema", return_value=False):
            tools = await tool_manager._convert_operation_schema_to_tools(
                operation_schema
            )
...
@pytest.mark.asyncio
async def test_convert_operation_schema_to_tools_meta(
    mock_schema_index, mock_schema_parser
):
    ...
    with patch(
        ...
        tools = await tool_manager._convert_operation_schema_to_tools(
            operation_schema, is_creating_meta_tool=False
        )
        assert len(tools) == 0

        # Test with is_creating_meta_tool=True (should be included)
        tools = await tool_manager._convert_operation_schema_to_tools(
            operation_schema, is_creating_meta_tool=True
        )
        assert len(tools) == 1
...
@pytest.mark.asyncio
async def test_convert_operation_schema_to_tools_with_model_schema(
    mock_schema_index, mock_schema_parser, mock_client
):
    ...
    with patch(
        ...
        with patch.object(tool_manager, "_needs_model_schema", return_value=True):
            with patch.object(
                tool_manager,
                "_can_call_associated_meta_operation_schema",
                return_value=True,
            ):
                with patch.object(
                    tool_manager, "_populate_model_schema_with_meta_call"
                ) as mock_populate:
                    with patch.object(
                        tool_manager, "_should_meta_tool_be_added", return_value=False
                    ):
...
async def test_populate_model_schema_with_meta_call(mock_schema_index, mock_client):
    ...
    with patch(
        ...
        operation_schema = {"endpoint": "/tickets", "method": "post"}

        input_schema = {"properties": {"model": {"type": "string"}}}

        await tool_manager._populate_model_schema_with_meta_call(
            operation_schema, input_schema
        )

        # Check that call_associated_meta_endpoint was called
        mock_client.call_associated_meta_endpoint.assert_awaited_once_with(
            "/tickets", "post"
        )
...
def test_build_request_from_schema_and_arguments(mock_schema_index):
    ...
    with patch(
        ...
        request_meta = tool_manager._build_request_from_schema_and_arguments(
            schema, arguments
        )
...
async def test_call_tool_success(mock_schema_index, mock_client):
    ...
    with patch(
        ...
        with patch.object(
            tool_manager, "_build_request_from_schema_and_arguments"
        ) as mock_build:
            request_meta = RequestMeta(
                method="GET", path="/tickets", query_params={"page": 1}, body_params={}
            )
...
async def test_ignore_tools_with_download_in_name(mock_schema_index, mock_client):
    ...
    with patch(
        ...
        with patch.object(
            ...
            mock_schema_index.get_all_operation_schemas.return_value = [
                operation_schema
            ]
